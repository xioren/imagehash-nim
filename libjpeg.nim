...
type
  Boolean = (when defined(windows): uint8 else: cint)
  ...
  JpegCompress = object
    ...
    arithDcL: array[NUM_ARITH_TBLS, uint8]
    arithDcU: array[NUM_ARITH_TBLS, uint8]
    arithAcK: array[NUM_ARITH_TBLS, uint8]
    numScans: cint
    scanInfo: pointer
    ...
    jfifMajorVersion, jfifMinorVersion, densityUnit: uint8
  ...
  JpegDecompress = object
    ...
    totalIMCURows: cuint
    sampleRangeLimit: ptr UncheckedArray[uint8]
    compsInScan: cint
    curCompInfo: array[MAX_COMPS_IN_SCAN, pointer]
    mCUsPerRow, mCURowsInScan: cuint
    blocksInMCU: cint
  ...
  JSAMPROW = ptr UncheckedArray[uint8]
...
when after80:
  proc jpeg_mem_src(dinfo: ptr JpegDecompress, inbuffer: ptr uint8, insize: culong)
...
template readJPEGImpl(dctMethod: JpegDCTMethod,
  ...
  var
    ...
    rows = newSeq[ptr UncheckedArray[uint8]](r.height)
    address = cast[int](addr r[0])
  let rowWidth = r.width * sizeof colorType r

  for row in 0..rows.high:
    rows[row] = cast[ptr UncheckedArray[uint8]](address)
    address += rowWidth
...
proc readJPEG*[T: Color](data: openArray[char], dctMethod = jDCTmISlow,
  fancyUpsampling = false, blockSmoothing = true): Image[T] =
  readJPEGImpl dctMethod, fancyUpsampling, blockSmoothing:
    jpeg_mem_src addr dinfo, cast[ptr uint8](unsafeAddr source[0]), source.len.culong
...
proc initDestination[T](cinfo: ptr JpegCompress) =
  ...
  a.pub.nextOutputByte = cast[ptr uint8](addr a.buffer[0])
...
proc emptyOutputBuffer[T](cinfo: ptr JpegCompress): Boolean =
  ...
  a.pub.nextOutputByte = cast[ptr uint8](addr a.buffer[][size])
...
template writeJPEGImpl[T: Color](img: Image[T], quality: int,
  ...
  var
    rows = newSeq[ptr UncheckedArray[uint8]](i.height)
