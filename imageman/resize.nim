...
proc lanczosKernel(x: float, a: int): float =
  if x == 0.0:
    return 1.0
  elif abs(x) < a.float:
    return a.float * sin(Pi * x) * sin(Pi * x / a.float) / (Pi * Pi * x * x)
  else:
    return 0.0


proc resizedLanczos*[T: Color](img: Image[T], w, h: int, a: int = 2): Image[T] =
  result.initImage w, h
  let
    xr = img.w.float / w.float
    yr = img.h.float / h.float
  ...
  for j in 0..<h:
    for i in 0..<w:
      ...
      for ky in max(0, int(srcY) - a + 1) ..< min(img.h, int(srcY) + a):
        for kx in max(0, int(srcX) - a + 1) ..< min(img.w, int(srcX) + a):
          let
            dx = srcX - kx.float
            dy = srcY - ky.float
            L = lanczosKernel(dx, a) * lanczosKernel(dy, a)
          let pixel = img[kx, ky]
          rSum += pixel.r.float * L
          gSum += pixel.g.float * L
          bSum += pixel.b.float * L
          norm += L
      
      if norm > 0.0:
        result[i, j] = componentType(T) ([uint8(rSum / norm), uint8(gSum / norm), uint8(bSum / norm)])
      else:
        result[i, j] = componentType(T) ([0, 0, 0])
